...
class Admin(commands.Cog):
	...
	async def reload_cog(self, ctx, extension):
		embed = discord.Embed(title='Success!', description=f'{extension} is reloaded.',
										color=self.color)
		try:
			self.bot.reload_extension(f'{self.main_directory}.{extension}')
		except:
			try:
				self.bot.reload_extension(f'{self.main_directory}.game_files.{extension}')
			except:
				pass
			else:
				await ctx.send(embed=embed)
	...
	async def restart(self, ctx):
		for filename in os.listdir(f'./{self.main_directory}'):
			if filename.endswith('.py'):
				self.bot.unload_extension(f'{self.main_directory}.{filename[:-3]}')
